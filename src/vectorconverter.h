 ...
 * - Image → SVG (raster trace): bundles potrace or autotrace CLI
...
public:
    ...
    // (poppler_bin, potrace, autotrace are looked up here first, then PATH)
    void setToolsDir(const QString &dir);
    QString toolsDir() const { return m_toolsDir; }

    // Check which tools are available
    bool hasPdftocairo() const;
    bool hasPotrace() const;
    bool hasAutotrace() const;
    ...
    // Image → SVG (raster trace, via potrace or autotrace)
    // Used for pages that are raster images, or for "SVG (Traced)" output mode
    void convertImageToSvg(const QString &imagePath,
                           const QString &outputPath,
                           bool colorTrace = false);
